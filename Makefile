...
ifneq ($(HAS_SERVER),)
	...
	$(GO) vet -vettool=$(GOBIN)/mattermost-govet -license ./server/...
	# Yet unsure how to even check `lt`, since `Package 'opusfile' not found` on some platforms.
	# cd lt && $(GO) vet -vettool=$(GOBIN)/mattermost-govet -license ./...
endif
