...
## Start LiveKit server in Docker with auto-detected LAN IP (plaintext ws://, for local dev on http://localhost)
.PHONY: livekit-docker-start
livekit-docker-start:
	LIVEKIT_NODE_IP=$$(ipconfig getifaddr en0) docker compose -f docker-compose.livekit.yaml up livekit-dev

## Stop LiveKit server
.PHONY: livekit-docker-stop
livekit-docker-stop:
	LIVEKIT_NODE_IP=$$(ipconfig getifaddr en0) docker compose -f docker-compose.livekit.yaml down livekit-dev

## Start LiveKit + Caddy TLS proxy (wss://, for external LAN machines on HTTPS Mattermost). Requires LIVEKIT_TLS_CERT and LIVEKIT_TLS_KEY.
.PHONY: livekit-tls-docker-start
livekit-tls-docker-start:
	@if [ ! -f "$(LIVEKIT_TLS_CERT)" ] || [ ! -f "$(LIVEKIT_TLS_KEY)" ]; then \
		echo "ERROR: set LIVEKIT_TLS_CERT and LIVEKIT_TLS_KEY to existing cert/key files."; \
		echo "  Tip: reuse the same self-signed cert your Mattermost server uses (already trusted by your clients)."; \
		echo "  Example: make livekit-tls-docker-start LIVEKIT_TLS_CERT=/path/to/cert.pem LIVEKIT_TLS_KEY=/path/to/key.pem"; \
		exit 1; \
	fi
	@echo ">> Set the plugin LiveKit URL to wss://$$(ipconfig getifaddr en0):7443 (System Console > Calls, or MM_CALLS_LIVEKIT_URL)"
	LIVEKIT_NODE_IP=$$(ipconfig getifaddr en0) \
		LIVEKIT_TLS_CERT="$(abspath $(LIVEKIT_TLS_CERT))" \
		LIVEKIT_TLS_KEY="$(abspath $(LIVEKIT_TLS_KEY))" \
		docker compose -f docker-compose.livekit.yaml up livekit-dev livekit-proxy

## Stop LiveKit + Caddy TLS proxy
.PHONY: livekit-tls-docker-stop
