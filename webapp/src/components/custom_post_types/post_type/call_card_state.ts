...
// answered. numParticipants closes that gap: a ringing call has only the caller in it, and the
...
export function getCallCardState(callProps: CallsPostProps, numParticipants: number): CallCardState {
    ...
    if (callProps.call_status === CallPostStatus.Calling && numParticipants <= 1) {
